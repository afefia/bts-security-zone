...
class AddConductRecordScreen extends StatefulWidget {
  ...
  @override
  State<AddConductRecordScreen> createState() => _AddConductRecordScreenState();
}
...
class _AddConductRecordScreenState extends State<AddConductRecordScreen> {
    ...
    try {
      ...
      if (user != null) {
        ...
        if (mounted) {
          setState(() {
            _currentUserName =
                data?['full_name'] as String? ?? user.email ?? 'Unknown';
  ...
  Future<void> _submit() async {
    ...
    // Block adding records to terminated or suspended recruits
    if (widget.recruit.status == 'terminated') {
      setState(() {
        _errorMessage =
            'This recruit has been terminated. You cannot add records to a terminated recruit.';
      });
      return;
    }
    if (widget.recruit.status == 'suspended') {
      final confirm = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          backgroundColor: AppTheme.cardBg,
          title: const Text('Recruit is Suspended'),
          content: const Text(
            'This recruit is currently suspended. Are you sure you want to add a record?',
          ),
          actions: [
            TextButton(
    ...
    try {
      ...
      // Use the auto-fetched current user name, fallback
      final reportedBy = _currentUserName ?? 'Unknown User';
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border:
                        Border.all(color: AppTheme.steelBlue.withOpacity(0.4)),
                  ),
                  child: Row(
                    children: [
                      CircleAvatar(
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(10),
                    border:
                        Border.all(color: AppTheme.steelBlue.withOpacity(0.5)),
                  ),
                  child: Row(
                    children: [
