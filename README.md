...
### Form fields — fill their row, but never touch the screen edge

Text fields are the one place that should fill available width — that's the standard, expected pattern (every banking app, every email client works this way), so `TextFormField` doesn't need a wrapper or special treatment. What needs to be consistent instead is the **margin around the field**, not the field's own width:
